...
const SEO: React.FC<HelmetProps> = ({ title, description, children, url, image, published, pathname, timeToRead }) => {
  ...
  return (
    <Helmet title={title || site.title} htmlAttributes={{ lang: "en" }} script={themeUIDarkModeWorkaroundScript} meta={metaTags}>
      <link rel="stylesheet" href="https://use.typekit.net/kov5cya.css" />
      {children}
    </Helmet>
  );
};
